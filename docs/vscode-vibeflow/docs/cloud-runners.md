...
**Open terminal** (available throughout the wizard) attaches a native VS Code terminal directly to the runner's pod — the same session the agent runs in. Type, scroll, and resize as usual; close the terminal tab to disconnect. Terminal access follows the same owner-only rule as Start/Stop/Delete.
