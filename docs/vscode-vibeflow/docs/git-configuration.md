...
Cloud Runners clone and push your repositories from the cloud, so they need git credentials of their own — separate from whatever credentials your local IDE uses. The **Git Configuration** tab in VibeFlow Settings is where you add and manage those credentials, called *git providers*.
...
Tokens and SSH keys are stored **on the VibeFlow server**, never on your machine and never in VS Code settings. After you save a provider, the credential is write-only: no screen in the extension ever displays it again. Lists and detail views show only the provider's name, git host, and authentication type.
