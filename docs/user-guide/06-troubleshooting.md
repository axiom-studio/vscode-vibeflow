...
**What it means**: The extension can't reach VibeFlow Cloud with a valid API key. Either the Setup wizard never finished, or your API key has been revoked / rotated server-side.
...
**What it means**: The wizard runs paste-hygiene + minimum-length checks on every provider key (issues #2174 / #2179). Two failures are common:
...
**Fix**: Re-copy the key directly from the provider's dashboard. Paste it raw — no `"…"` wrapping, no `[…]`, no trailing newline.
...
- **Cancel immediately** — right-click the row → **Cancel Starting Session** (`vibeflow.cancelStartingPending`). Kills the pending row at once.
...
**What it means**: This is by far the most-reported chat-first symptom (issue #2305). It almost always means: your chat-first session is using the **vscode** headless backing, which only supports a single chat turn before the agent process exits. Turn 2 has nowhere to send the message — the panel waits, the panel waits forever.
...
1. Confirm you're on v1.0.3 or later (check the **VS Code Extensions** view).
2. Verify your API key is set: **VibeFlow: Settings** → **Connection** tab. If the field is empty, paste your key.
...
**Fix**: Update the extension from the VS Code Marketplace. If you're already on v1.0.3+ and still seeing it, you may have a stale cached install — reload VS Code (`Cmd/Ctrl+Shift+P` → **Developer: Reload Window**) or fully reinstall the extension.
...
**What it means**: Almost always: the provider binary isn't on `PATH` for the shell VS Code spawned. The agent process tries to exec `claude` (or `codex`, `gemini`, etc.), `exec` fails, the terminal closes before you can read the error.
...
3. If the command works in your shell but not in VS Code's terminal, your `PATH` is set in a shell init file VS Code doesn't load. Either move the PATH export to `~/.zshenv` / `~/.bash_profile`, or set `vibeflow.cli.binaryPath` to an absolute path.
...
## MCP server doesn't appear in Copilot / Continue / Cody

**What it means**: The VibeFlow extension registers itself as an MCP server via VS Code's MCP API so other Copilot-like extensions in your IDE can discover and call VibeFlow tools. The registration happens once at extension activation — if VibeFlow activated before the other extension was installed or upgraded, the other extension never saw the registration.

**Fix**: Fully reload VS Code (`Cmd/Ctrl+Shift+P` → **Developer: Reload Window**) — not just the VibeFlow view. After reload, the consuming extension should see `vibeflow` in its MCP server list. If it still doesn't, check that the consuming extension supports VS Code's native MCP API (some Copilot-likes ship their own discovery protocol).
...
1. Run **VibeFlow: Report an Issue…** from the Command Palette. It opens a pre-populated GitHub issue with your extension version, VS Code version, and OS pre-filled.
