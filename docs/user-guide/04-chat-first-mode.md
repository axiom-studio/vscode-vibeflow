...
**TL;DR**: Chat-First Mode lets you talk to a VibeFlow agent the way you talk to Copilot Chat or Cursor — type a message, the agent answers, type a follow-up, it keeps going. No work item required to start a conversation. Under the hood it always runs in YOLO (skip-permissions) mode and prefers `tmux` as its runtime backing. This is the most powerful — and the most opinionated — way to use the extension.
...
Pre-chat-first VibeFlow forced every interaction through the work-item pipeline. That's right when work is well-scoped, but friction-heavy when:
...
- Unix only — silently ignored on Windows.
...
- **Single-turn only.** The provider CLI under `--print`/`--input-format stream-json` exits after one response. The hidden-terminal backing has no mechanism to respawn it for turn 2 — subsequent messages hang at "Working…" forever. This is the historical issue **#2305**.
...
Provider CLIs — `claude`, `codex`, `gemini`, `cursor`, `qwen` — are one-shot under their headless `--print` / `--input-format stream-json` modes. They read one user message from stdin, stream the response, then exit. They are not long-lived processes — at odds with "multi-turn chat," which needs a running process to talk to.
...
- **"Why does each response take a beat longer than Copilot?"** A small portion of each turn re-hydrates session state from the provider — the cost of using one-shot CLIs for multi-turn chat.
- **"Can I attach to the agent process between turns?"** Only with `tmux`, and only briefly — the process is short-lived. There is no idle agent state to spy on.
...
- **The work is autonomous and queue-shaped.** "Burn through this backlog of 12 todos overnight" is not a chat workflow — there's nobody to chat with.
- **You need a full audit trail of permission decisions.** YOLO does not produce one.
...
Work items the chat-first session created persist on the server — they're not tied to the session's lifetime.
