...
**Gotchas**: Include the scheme; no trailing slash. Changes invalidate your stored API key — re-run **`VibeFlow: Setup`**.
...
**When to change it**: You don't have an Anthropic key but do have OpenAI / Google / Cursor. Set this to your daily-driver provider so launches are one click less.
...
| `vibeflow` | Reattach in **YOLO mode** (`--dangerously-skip-permissions`) — no prompts. |

**When to change it**: You routinely launch in YOLO mode and want restarts to preserve that.

**Gotchas**: `vibeflow` is **risky** — reattached sessions get full permission with no prompts. Only set it if (a) your prior session was YOLO and (b) you trust the agent to keep running unattended. No per-session override on reattach — applies to every session that comes back.
...
| Value | Behavior |
|---|---|
| `auto` | tmux when available (Mac/Linux with tmux installed), otherwise hidden VS Code terminal. **Recommended** — tmux is the only backing that supports multi-turn chat today. |
...
**Gotchas**: Doesn't affect prompts, errors, or security/QA review notifications — only the "work item complete" toast.
...
**What it does**: How often the extension re-fetches data to refresh sidebar views. Lower = snappier UI, higher = less CPU and network.
...
**Gotchas**: **Purely cosmetic** — changes the inline preview only. Each diff has an **Open in Editor** button that opens VS Code's native diff editor for proper review. Use that button when a diff matters.
...
**Gotchas**: Off by default — worktrees consume disk and clutter `git worktree list`. Pair with `vibeflow.worktree.cleanupOnKill = always` for fully automatic lifecycle.
